...
public class Game extends STSerializableObject implements SpaceTraderGame, SystemTracker, CurrentSystemMgr
{
	...
	// Current Selections
	private Difficulty _difficulty = Difficulty.Normal; // Difficulty
	// level
	private boolean _autoSave = false;
	private boolean _easyEncounters = false;
	private GameEndType _endStatus = GameEndType.NA;
	private EncounterType _encounterType = EncounterType.FromInt(0); // Type
	// of
	// current
	// encounter
...
@CheatCode	private final GameCheats _cheats;
	...
	private boolean DetermineNonRandomEncounter()
	{
				...
				&& getQuestStatusSpaceMonster() == SpecialEvent.StatusSpaceMonsterAtAcamar)
		{
			...
			setEncounterType(Commander().getShip().Cloaked() ? EncounterType.SpaceMonsterIgnore
		...
		{
			setOpponent(Scarab());
			setEncounterType(Commander().getShip().Cloaked() ? EncounterType.ScarabIgnore
				...
				&& getQuestStatusDragonfly() == SpecialEvent.StatusDragonflyFlyZalkon)
		{
			...
			setEncounterType(Commander().getShip().Cloaked() ? EncounterType.DragonflyIgnore
				...
				&& getQuestStatusPrincess() == SpecialEvent.StatusPrincessFlyQonos)
		{
			...
			setEncounterType(Commander().getShip().Cloaked() ? EncounterType.ScorpionIgnore
					: EncounterType.ScorpionAttack);
			showEncounter = true;
		}
	...
	private boolean DetermineTraderEncounter()
	{
		...
		if (!getOpponent().Cloaked()
				...
				&& !((getEncounterType() == EncounterType.TraderBuy || getEncounterType() == EncounterType.TraderSell) && Options()
	...
	public EncounterResult EncounterExecuteAction()
	{
		...
		} else
		{
			...
			if (getEncounterCmdrFleeing()
					&& (Difficulty() == Difficulty.Beginner || (Functions.GetRandom(7) + Commander()
					...
					if (damage > 0)
					{
						...
						if (defender.ReactorOnBoard())
							damage *= (int)(1 + (Difficulty().CastToInt() + 1)
									* (Difficulty().CastToInt() < Difficulty.Normal.CastToInt() ? 0.25
											: 0.33));
						...
						if (damage > 0)
						{
							...
							damage = Math.min(damage, defender.HullStrength()
									/ (defender.CommandersShip() ? Math.max(1, Difficulty.Impossible
											.CastToInt()
	...
	public void EncounterMeet()
	{
		...
		if (GuiFacade.alert(initialAlert) == DialogResult.Yes)
		{
			...
			Commander().Skills()[skill] = Math.min(Consts.MaxSkill, Commander().Skills()[skill]
					+ (Difficulty().CastToInt() <= Difficulty.Normal.CastToInt() ? 2 : 1));
	...
	private void EncounterScoop()
	{
		...
		if ((Difficulty().CastToInt() < Difficulty.Normal.CastToInt() || Functions
	...
	public boolean EncounterVerifyBribe()
	{
				...
				|| GuiFacade.alert(AlertType.EncounterPoliceNothingIllegal) == DialogResult.Yes)
		{
			...
			int diffMod = 10 + 5 * (Difficulty.Impossible.CastToInt() - Difficulty().CastToInt());
			int passMod = Commander().getShip().IllegalSpecialCargo() ? (Difficulty().CastToInt() <= Difficulty.Normal
					.CastToInt() ? 2 : 3)
					: 1;
				...
				|| GuiFacade.alert(AlertType.EncounterPoliceNothingIllegal) == DialogResult.Yes)
		{
					...
					|| getEncounterType() == EncounterType.MarieCelestePolice)
			{
				...
				int scoreMin = getEncounterType() == EncounterType.PoliceInspect ? Consts.PoliceRecordScoreDubious
						- (Difficulty().CastToInt() < Difficulty.Normal.CastToInt() ? 0 : 1)
		...
		if (Commander().getPoliceRecordScore() > Consts.PoliceRecordScoreClean)
			...
			Commander().setPoliceRecordScore(
					Math.min(Consts.PoliceRecordScoreDubious, Commander().getPoliceRecordScore()
							+ num
							/ (Difficulty().CastToInt() <= Difficulty.Normal.CastToInt() ? 1
	...
	public void SelectNextSystemWithinRange(boolean forward)
	{
		...
		if (dest.length > 0)
		{
			int index = Util.BruteSeek(dest, WarpSystemId().CastToInt());
	...
	public String EncounterAction()
	{
		...
		{
			if (getEncounterType() == EncounterType.PirateSurrender
					|| getEncounterType() == EncounterType.TraderSurrender)
